function showRegularTicketContent() {
    var regularTicketContent = document.getElementById("regularTicketContent");
    var passContent = document.getElementById("passContent");

    // Hide pass content if it's currently visible and the regular ticket content is not active
    if (passContent.style.display === "block" && regularTicketContent.style.display !== "block") {
        passContent.style.display = "none";
    }

    // Toggle regular ticket content visibility only if it's not already visible
    if (regularTicketContent.style.display !== "block") {
        regularTicketContent.style.display = "block";
    }
}
...
function showPassForm(){
    ...
    // Hide regular ticket content if it's currently visible and the pass content is not active
    if (regularTicketContent.style.display === "block" && passContent.style.display !== "block") {
        regularTicketContent.style.display = "none";
    }

    // Toggle pass content visibility only if it's not already visible
    if (passContent.style.display !== "block") {
...
function confirmTicket(boardingStop, destinationStop, adults, totalFare, availableBuses) {
    ...
    submitButton.addEventListener("click", function () {
        ...
        if (password === "1173") {
            generateTicket(boardingStop, destinationStop, adults, totalFare, availableBuses);
        } else {
